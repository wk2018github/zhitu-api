...
$(function () {
    ...
    $.ajax({
        url: '/zhitu-api/dataSet/queryTableColumn',
        type: 'post',
        async: false,
...
var initTableData = function initTableData(_ref) {
    ...
    $.ajax({
        type: "post",
        url: "/zhitu-api/dataSet/queryTableData",
...
$('#sure-btn').click(function () {
    if(arr.length==0){
        ...
        if ($('input[name="database-type"]:checked').val() == '0') {
            $.ajax({
                url: ' /zhitu-api/dataSet/queryLocalTableColumnData',
                type: 'post',
                contentType: 'application/json',
                data: JSON.stringify({
                ...
                success: function success(res) {
                    if (res.code == '000000') {
                        window.location.href = '../newData/thirdStep.html?uping=1&taskId='+res.data.taskId+'&columnNames='+str+'&id='+res.data.id+'&databaseType='+databaseType+'&charset='+charset+'&user='+user+'&tableName='+tableName+'&port='+port+'&password='+password+'&host='+host+'&dbName='+dbName;
                    }
                }
            });
        } else {
            $.ajax({
                url: '/zhitu-api/dataSet/saveSqlDb',
                type: 'post',
                contentType: 'application/json',
                data: JSON.stringify({
                    charset: charset, //  数据字符集   string  @mock=string
                ...
                success: function success(res) {
                    if (res.code == '000000') {
                        window.location.href = 'userDataSet.html?dbName=' + dbName + '&host=' + host + '&password=' + password + '&port=' + port + '&tableName=' + tableName + '&user=' + user + '&charset=' + charset + '&databaseType=' + databaseType + '&columnNames=' + str+'&id='+res.data.id+'&source='+source+'&source1=2';
                    }
                }
            });
        }
