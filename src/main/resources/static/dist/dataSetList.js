...
var visualPage = function visualPage(dataSetId) {
...
//删除(批量删除)
var realDel = function realDel(arrs) {
    $.ajax({
        type: "post",
...
//编辑
var editDataSet = function editDataSet() {
...
$("#doDel").on("click", function () {
    ...
    layer.confirm('是否删除选中的数据集?', {
        ...
        try {
            for (var _iterator = checkStatus.data[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var t = _step.value;
...
table.on('tool', function (obj) {
    ...
    if (layEvent === 'visualPage') {
        ...
        //编辑
        //弹出层编辑
        var _html = '<div class="diy-layer-wrap"><div class="form-wrap"><div class="edit-item">' + '<label for="dataset-name">数据集名:</label><input type="text" class="layui-input" id="dataset-name" value="" /></div> ' + '<div class="edit-item"><label for="dataset-name">描述:</label><textarea class="layui-textarea" id="description"></textarea></div> </div></div>';
